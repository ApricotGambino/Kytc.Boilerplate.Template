...
public static class PaginationExtensions
{
    ...
    public static List<T> GetNextPageUsingKeysetPaginationasdfasdfasdf<T, TKey>(this DbSet<T> list, Func<T, TKey> keySelector, TKey lastValue, int lastId, int pageSize)
        where T : Log
        where TKey : IComparable
    {
        //NOTE: This method is designed to be used after an initial Offset pagination query, or at least an already ordered data set.
        ...
        return list
          ...
          //.Where(p => (p.Message == lastValue && p.Id > lastId) || string.Compare(p.Message, lastValue) > 0)
          .OrderBy(keySelector)
          .Take(pageSize)
          .ToList();
    }
